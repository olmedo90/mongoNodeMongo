...
const validacion = require('../conf/validaciones.js');
// leer db
routes.get('/', async(req, res)=>{
    try {
        const arrayUser= await User.find();
        res.render('users.html',{
            titulo:'mi Usuario', arrayUser
        })
    } catch (error) {
        console.log(error);
    }
...
routes.post('/', async(req, res)=>{
    ...
    const data = validacion.validUsers(req.body);
    //console.log(data);
    if(data.status){
        try {
            const clientDB = new User(req.body);
            await clientDB.save();
            res.redirect('/users');
